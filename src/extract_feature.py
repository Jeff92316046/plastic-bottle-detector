...
for (i, trnPath) in enumerate(trnPaths):
    ...
    imageID = trnPath[trnPath.rfind("\\") + 1:].replace(".jpg", "")
    # load the annotation file associated with the image and extract the bounding box
    p = f"{conf['image_annotations']}/{imageID}.txt"
    with open(p,"r") as file:
        for j in file.readlines():
            values = j.strip().split()
            if len(values) != 5:
                break
            tuple_data = (int(values[0]),) + tuple(map(float, values[1:]))
            bb = helpers.yolo_to_bbox(tuple_data,image.shape[1],image.shape[0])
...
pbar = progressbar.ProgressBar(maxval=len(np.arange(0, conf["num_distraction_images"])), widgets=widgets).start()
